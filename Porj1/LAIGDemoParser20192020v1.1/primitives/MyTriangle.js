...
class MyTriangle extends CGFobject {
	...
	initBuffers() {
		...
		//Internal angles
		this.cosB = (Math.pow(this.A, 2) - Math.pow(this.B, 2) + Math.pow(this.C, 2)) / (2 * Math.pow(this.A, 2));
		this.sinB = Math.sqrt(1 - Math.pow(this.cosB, 2));

		//AB side
		let AB =[	this.x2 - this.x1, 
					this.y2 - this.y1, 
					this.z2 - this.z1
		];

		//AC side
		let AC =[	this.x3 - this.x1, 
					this.y3 - this.y1, 
					this.z3 - this.z1
		];

		//??
		let n =[(AB[1] * AC[2]) - (AC[1] * AB[2]),
				(AB[2] * AC[0]) - (AC[2] * AB[0]),
				(AB[0] * AC[1]) - (AC[0] * AB[1])
		];
		...
		this.textCoords = [
			this.C - this.A * this.cosB, this.A * this.sinB,
			0, 0,
			this.C, 0
        ];
